...
impl NonFungibleTokenRoyalty for Contract {
    ...
    fn nft_payout(&self, token_id: TokenId, balance: U128, max_len_payout: u32) -> Payout {
        ...
        payout_object.payout.insert(
            ...
            royalty_to_payout(10_000u32.saturating_sub(total_perpetual), balance_u128),
        );
    ...
    fn nft_transfer_payout(
        ...
        // assert is approved account
        require!(
            self.nft_is_approved(token_id.clone(), sender_id.clone(), None),
            "Can't transfer if not an approved account_id"
        );
