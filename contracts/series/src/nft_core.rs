...
impl NonFungibleTokenCore for Contract {
    ...
    fn nft_transfer(
        ...
        let content_id = self
            ...
            .unwrap()
            .content_id;
        self.treasury_update_holder(content_id, token_id.clone(), receiver_id.clone());
    ...
    fn nft_transfer_call(
        ...
        ext_non_fungible_token_receiver::ext(receiver_id.clone())
            ...
            .then(
                Self::ext(env::current_account_id())
                    .with_static_gas(GAS_FOR_RESOLVE_TRANSFER)
                    .on_nft_transfer_resolved(content_id, token_id, receiver_id),
            )
            .into()
    }
...
impl Contract {
    #[private]
    pub fn on_nft_transfer_resolved(
        &self,
        content_id: String,
        token_id: String,
        new_holder: AccountId,
    ) -> bool {
        // assert token was transferred successfully
        if let PromiseResult::Successful(value) = env::promise_result(0) {
            if let Ok(res) = near_sdk::serde_json::from_slice::<bool>(&value) {
                if res {
                    // if everything went allright in prev promise (res == true), update holder in treasury
                    // return self.treasury_update_holder(content_id, token_id, new_holder);
                    self.treasury_update_holder(content_id, token_id, new_holder);

                    // return nft_resolve_transfer result
                    return true;
                }
            }
        }
