...
import { IWalletSelectorProps, IServerSideProps, isServerSideProps, IQueryResponseKindCustom } from "../../types/getSigner.types";
import { Account, KeyPair, connect } from "near-api-js";
import { InMemoryKeyStore } from "near-api-js/lib/key_stores";
import { JsonRpcProvider } from "near-api-js/lib/providers";

export const getSigner = async (props: IServerSideProps | IWalletSelectorProps): Promise<ISigner> => {
	// 1. Type guard
	// describe two scenarios:
	if (isServerSideProps(props)) {
		const { network, accountId, privateKey } = props
